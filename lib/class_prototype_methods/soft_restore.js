...
import documentSoftRestore from '../utils/document_soft_restore.js';
const wrapCallback = Module.modules.core.utils.wrapCallback;

function softRestore(args = {}, callback) {
  let doc = this;
  ...
  // If we can just restore a document without calling the meteor method. We may
  // be on the server or the collection may be local.
    ...
    try {
      ...
      return wrappedCallback(
        undefined,
        documentSoftRestore(methodArgs)
      );
    }
    catch (error) {
      wrappedCallback(error);
